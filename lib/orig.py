...
▀▀▄▄▄▀▀▄▄▄▀▀▄▄▀▀▄▄▄▀▀▀▀▀▀▀▀▀▄▄▀▄▄▀▄▄▄▀▄▄▄▀▄▄▄▀"""
    ...
    def is_target_locked(x, y):
        ...
        threshold = 5
        return True if half_screen_width - threshold <= x <= half_screen_width + threshold and half_screen_height - threshold <= y <= half_screen_height + threshold else False
    ...
    def interpolate_coordinates_from_center(absolute_coordinates, scale):
        diff_x = (absolute_coordinates[0] - half_screen_width) * scale/Aimbot.pixel_increment
        diff_y = (absolute_coordinates[1] - half_screen_height) * scale/Aimbot.pixel_increment
    ...
    def start(self):
        ...
        while True:
            ...
            if self.collect_data and time.perf_counter() - collect_pause > 1 and Aimbot.is_targeted() and Aimbot.is_aimbot_enabled() and not player_in_frame: #screenshots can only be taken every 1 second
                ...
                pass
            #ctypes.windll.kernel32.SetConsoleTitleW(f"LUNAR-v2 PRESET ORIG (FRAME TIME: {str(time.perf_counter() - start_time)}) (FPS: {str(int(1/(time.perf_counter() - start_time)))})")
